...
docker run -d \
  ...
  ghcr.io/archarcade/cedar-agent-frontend:latest
```
...
docker buildx build \
  ...
  -t ghcr.io/archarcade/cedar-agent:latest \
  --push \
  .
```
...
export DOCKER_ORG=anonymous-author
...
docker pull --platform linux/amd64 ghcr.io/archarcade/cedar-agent:latest
